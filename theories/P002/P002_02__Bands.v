...
(*@doc.pl@[[This layer specializes the FM digit constraints to fixed machine windows.]]@*)

(*@doc.pl@[[Only the pieces that matter for `compile_fm_family_correct` are kept central here: band-local booleanity, band-local non-adjacency, and the weighted linear decoder that turns a valid digit window into the corresponding `band_code`.]]@*)
...
(*@inline@[[The boolean and non-adjacency emitters are equation-valued because later compilation needs honest local constraints, not raw term payloads.]]@*)
...
(*@inline@[[The canonical indicator valuation attached to a band support solves every emitted digit equation for that band. This is the local correctness fact that the later compiler will need when it instantiates digit witnesses coming from a concrete FM run.]]@*)
...
(*@inline@[[Once the target variable is kept outside the active digit window, the auxiliary valuation agrees pointwise with the pure indicator valuation on every emitted digit index. This is the separation fact that lets the decoder equation reuse the support semantics without accidental self-interference through the target channel.]]@*)
(*@unicodemath@[[target\_var \notin [offset+2,\,offset+K) \Rightarrow \forall k \in band\_digit\_indices(offset,K),]][[band\_decoder\_valuation(target\_var,offset,x)(k)=indicator\_valuation(band\_support(offset,x))(k).]]@*)
...
(*@inline@[[`band_decoder_correct` is the main arithmetic bridge of the file: under the canonical support valuation, the emitted linear decoder solves its equation and evaluates to the FM band code. This is exactly the band-local statement later needed when a full FM trace is compiled into one global cubic instance.]]@*)
(*@unicodemath@[[x < fib(K) \Rightarrow solves(band\_decoder\_equation(target,offset,K),\,band\_decoder\_valuation(target,offset,x))]][[\,\wedge\, \llbracket emit\_band\_decoder(target,offset,K)\rrbracket = band\_code(offset,x).]]@*)
