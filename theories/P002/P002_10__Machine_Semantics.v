...
(*@doc.pl@[[This layer sits directly on top of the fixed-witness trace family in `P002_09__Trace_Family`.]]@*)

(*@doc.pl@[[Its role is now strictly family-level: expose the open witness-search emitter, relate it to the existence of numeric FM trace witnesses, and package the internal compiler theorem `compile_fm_family_correct`.]]@*)

(*@doc.pl@[[No single-equation compression appears here. The compiler endpoint of the reset P002 project is a finite cubic family, not one aggregated cubic instance.]]@*)
...
(*@inline@[[`chosen_trace_bound` is the exact place where the remaining classicality of the unbounded compiler is concentrated. It does not choose a witness directly; it chooses only a numeric budget guaranteed to dominate some valid witness, after which the constructive bounded search layer takes over.]]@*)
(*@unicodemath@[[chosen\_trace\_bound(prog,input)\;:=\;\varepsilon B.\;\exists W\le B,\ FMValidTrace(prog,input,W).]]@*)
...
(*@inline@[[`emit_witness_search_family` is currently realized as a classical choice of a bound large enough to contain some valid witness, followed by the constructive bounded search compiler. This is enough to close the family-level theorem chain cleanly, but it should still be read as an intermediate search-family realization rather than the final syntax-driven unbounded compiler.]]@*)
(*@inline@[[`emit_bounded_witness_search_family` is the constructive sibling: given an explicit numeric bound, it performs primitive-recursive search over raw witness numbers and emits the corresponding fixed-witness family if one is found. This is the honest syntax-first search layer currently available in P002.]]@*)
...
(*@inline@[[`witness_search_family_payload_correct` now follows by case-splitting on witness existence and routing through the fixed-witness theorem `trace_family_correct`. The current open emitter is therefore concrete, though still semantically chosen rather than yet syntax-first.]]@*)
(*@unicodemath@[[FMTraceSearchPayload(prog,input)\Leftrightarrow cubic\_family\_yes(emit\_witness\_search\_family(prog,input)).]]@*)
(*@inline@[[`bounded_witness_search_family_payload_correct` isolates the constructive search theorem that does not need classical choice: within any explicit bound, the bounded search emitter is satisfiable exactly when some valid FM witness occurs below that bound.]]@*)
...
(*@inline@[[`compile_fm_family` is the honest compiler endpoint exported by the reset project: given an FM program and input, produce the cubic family whose satisfiability is intended to capture one accepting run.]]@*)
(*@unicodemath@[[compile\_fm\_family(prog,input) := emit\_witness\_search\_family(prog,input).]]@*)
...
(*@inline@[[`compile_fm_family_correct` is the new internal north star of P002: explicit FM witnesses exist exactly when the emitted cubic family is satisfiable. Every later Σ₁-completeness statement should factor through this theorem, not through any single-equation compression claim.]]@*)
(*@unicodemath@[[\exists W,\ FMValidTrace(prog,input,W)\Leftrightarrow cubic\_family\_yes(compile\_fm\_family(prog,input)).]]@*)
(*@inline@[[`compile_fm_family_upto_correct` is the constructive approximation to the open search theorem. It is completely syntax-driven: below any explicit bound, compilation and satisfiability exactly match bounded FM witness existence, with no appeal to classical witness choice.]]@*)
(*@inline@[[`compile_fm_family_exists_bound_correct` isolates the remaining unbounded step cleanly: unbounded FM witness existence is already equivalent to the existence of some satisfiable bounded compiled family. What remains classical in `compile_fm_family_correct` is therefore only the collapse of that outer existential over bounds into one open family instance.]]@*)
(*@inline@[[This theorem is the architectural hinge between the constructive and classical halves of reset P002. It says the syntax-driven bounded compiler is already extensionally complete for raw FM witness existence; the unbounded endpoint only packages the outer existential over bounds into one family-valued map.]]@*)
(*@unicodemath@[[\exists W,\ FMValidTrace(prog,input,W)\Leftrightarrow \exists B,\ cubic\_family\_yes(compile\_fm\_family\_upto(prog,input,B)).]]@*)
(*@inline@[[`compile_fm_family_yes_iff_exists_bound` is the same factorization stated directly at the family-satisfiability layer. It is the cleanest summary of the current architecture: the unbounded compiled family adds no new satisfiable instances beyond the bounded constructive core; it only hides the outer bound existential behind one classical wrapper.]]@*)
