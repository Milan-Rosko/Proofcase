...
└──────────────────────────────────────────────────────────────────────────────┘
┌──────────────────────────────────────────────────────────────────────────────┐
│                   Proofcase / P002_04__Machine_Definition                    │
└──────────────────────────────────────────────────────────────────────────────┘

  OVERVIEW

  This layer re-exports the structured Fibonacci-machine ISA from `D001_04`
  into the P002 namespace and packages the machine syntax facts that later
  arithmetization files use.

  P002 does not redefine the machine. It reflects the proved D001 hardware
  surface so later polynomial emitters can target a stable local API.
...
(*
│
│          The tag and projection helpers in this file are purely
│          descriptive. They do not change the FM instruction set
│          imported from D001; they expose small arithmetic handles
│          that later P002 emitters can inspect without pattern
│          matching on the full inductive syntax each time.
│
*)
(*instruction\_tag : \mathcal I \to \{0,1,2\},\qquad counter\_tag : \mathcal C*)
(*                                \to \{1,2\}.                                *)
...
(*
│
│          `fetch_instruction_at` is the ROM-facing lookup primitive
│          used by the later transition arithmetization. It makes the
│          program counter explicit as a raw natural index while
│          remaining definitionally equal to the underlying
│          `nth_error` access.
│
*)
(* fetch\_instruction\_at(prog,ip)=prog[ip]\ \text{(partial lookup in ROM)}.  *)
...
(*
│
│          The initial-state lemmas are the entry point from external
│          input into the FM semantics. They pin down the concrete
│          start configuration that later trace predicates and
│          compiler correctness statements must reproduce
│          arithmetically.
│
*)
(*                  initial\_state(input)=(1,input,0),\qquad                  *)
(*                   initial\_state2(r_1,r_2)=(1,r_1,r_2).                    *)
