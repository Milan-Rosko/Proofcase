...
(*@doc.pl@[[This layer now stops exactly at the family semantics needed for the reset P002 target.]]@*)

(*@doc.pl@[[Its role is narrow: fix the type of finite cubic families, the shared-valuation satisfaction relation for a whole family, and the basic list lemmas that the later trace emitter proofs use. The broken family-to-single-equation compression route is intentionally absent.]]@*)
...
(*@inline@[[An `equation_family` is the honest compilation target of the FM arithmetization: a finite list of atomic cubic equations, all read over one common valuation.]]@*)
(*@unicodemath@[[family\_satisfiable(\Gamma)\;\equiv\;\exists \rho.\ \forall E\in\Gamma,\ solves(E,\rho).]]@*)
...
(*@inline@[[`solves_all_nil`, `solves_all_cons`, `solves_all_singleton`, and `solves_all_app` are the only family combinators exported here because they are exactly the proof tools needed to decompose emitted trace families blockwise.]]@*)
