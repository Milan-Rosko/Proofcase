(*P002_05__Transition_Relation.v*)
...
└──────────────────────────────────────────────────────────────────────────────┘
┌──────────────────────────────────────────────────────────────────────────────┐
│                   Proofcase / P002_05__Transition_Relation                   │
└──────────────────────────────────────────────────────────────────────────────┘

  OVERVIEW

  This layer reflects the one-step Fibonacci-machine transition from
  `D001_05` into the P002 namespace and packages the observation predicates
  that the later polynomial emitters target.

  The step itself remains the proved D001 function. P002 adds only naming and
  bridge lemmas for encoded-state inspection.
...
(*
│
│          This file works entirely with the projected external
│          dynamics on encoded naturals. The current code `s` is first
│          decoded to a structured FM state, one proved D001 step is
│          executed there, and the result is re-encoded as the next
│          external code.
│
*)
(*    StepCode(prog,s)=encode\_state(step\_state(prog,decode\_state(s))).     *)
...
(*
│
│          The bridge lemmas here simply restate the proved D001
│          transition facts in P002 names. Their purpose is not new
│          semantics but namespace stability for the later arithmetic
│          compiler.
│
*)
...
(*
│
│          `next_state_on_encoded_state_bridge` is the key
│          compatibility fact for P002: on genuine FM image points,
│          stepping at the code level agrees exactly with stepping the
│          structured machine state and then re-encoding. This is the
│          theorem that lets later polynomial constraints talk about a
│          raw numeric successor while still meaning the intended
│          machine transition.
│
*)
(*                     state\_well\_formed(st)\Rightarrow                     *)
(*    StepCode(prog,encode\_state(st))=encode\_state(StepState(prog,st)).     *)
