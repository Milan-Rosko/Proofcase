...
└──────────────────────────────────────────────────────────────────────────────┘
┌──────────────────────────────────────────────────────────────────────────────┐
│                       Proofcase / P002_03__State_Codec                       │
└──────────────────────────────────────────────────────────────────────────────┘

  OVERVIEW

  This layer packages three already-computed band codes into one state-code
  equation.

  Its role for `compile_fm_family_correct` is narrow: it proves the additive
  aggregation step `state = ip_code + r1_code + r2_code`. It does not, by
  itself, arithmetize digit extraction or stand alone as a full state
  decoder.
...
(*
│
│          `emit_state_codec_rhs` is only the additive payload of the
│          state codec. The band-code arithmetic lives in `P002_02`;
│          this file merely sums those three pre-established channels.
│
*)
...
(*
│
│          The semantic content of the emitted state equation is
│          exactly one additive identity: the designated state
│          variable must equal the sum of the three band-code
│          variables. Nothing stronger is hidden here; all digit-level
│          meaning must already have been established upstream in the
│          band layer.
│
*)
(*              solves(state\_codec\_equation(s,i,r_1,r_2),\rho)              *)
(*               \Leftrightarrow equation\_wf(\cdots)\ \wedge\                *)
(*                    \rho(s)=\rho(i)+\rho(r_1)+\rho(r_2).                    *)
...
(*
│
│          `state_codec_equation_correct` should therefore be read as
│          an aggregation lemma over precomputed band-code variables,
│          not as a self-contained proof that raw digit variables
│          decode to a machine state.
│
*)
...
(*
│
│          `state_arithmetization_form` is the projection-facing
│          converse used later in P002: whenever a raw code is known
│          to lie in the valid FM image, decoding it and re-expanding
│          its three channels recovers the same arithmetic
│          decomposition. This is the precise point where the total
│          projection semantics of D001 is turned back into a usable
│          additive identity.
│
*)
(*             valid\_state\_code(s)\ \wedge\ decode\_state(s)=st             *)
(*     \Rightarrow s = r2\_code(st.r_2)+r1\_code(st.r_1)+ip\_code(st.ip).     *)
