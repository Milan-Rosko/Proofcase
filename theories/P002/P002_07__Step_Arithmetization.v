(*P002_07__Step_Arithmetization.v*)
...
└──────────────────────────────────────────────────────────────────────────────┘
┌──────────────────────────────────────────────────────────────────────────────┐
│                  Proofcase / P002_07__Step_Arithmetization                   │
└──────────────────────────────────────────────────────────────────────────────┘

  OVERVIEW

  This layer imports the proved D001 observation algebra for one
  Fibonacci-machine step and packages selector-gated cubic equations for the
  atomic transition constraints.

  The equations here do not yet aggregate a full step. They provide the
  reusable cubic atoms that the later compiler layer will combine.
...
(*
│
│          The selector emitters are the basic cubic atoms of the FM
│          compiler. They do not yet choose a branch by themselves;
│          instead they express “if the selector is active, then the
│          guarded relation must hold”, while later layers add the
│          selector-booleanity and exclusivity constraints.
│
*)
(* a\cdot x = 0,\quad a\cdot x = a\cdot y,\quad a\cdot t = a + a\cdot s,\quad *)
(*                            a\cdot x\cdot y = 0.                            *)
...
(*
│
│          The active/inactive correctness lemmas are the semantic
│          contract of the selector atoms. When the selector takes
│          value `1`, the intended guarded arithmetic relation is
│          enforced; when it takes value `0`, the corresponding
│          equation becomes vacuous.
│
*)
(*   a=1 \Rightarrow solves(E\_{guarded},\rho)\Leftrightarrow\text{guarded    *)
(*        relation},\qquad a=0 \Rightarrow solves(E\_{guarded},\rho).         *)
...
(*
│
│          The additive bridge theorems connect the local selector
│          atoms back to the real FM dynamics. They say that on
│          well-formed encoded states, one machine step changes the
│          external code exactly by the instruction-pointer delta plus
│          the register delta, with HALT collapsing to the IP-reset
│          case.
│
*)
(*          StepCode(prog,\encode\_state(st)) = \encode\_state(st) +          *)
(*     \Delta\_{IP}(prog,st) + \Delta\_{R}(prog,st)\ \text{(over }\mathbb     *)
(*                                 Z\text{)}.                                 *)
