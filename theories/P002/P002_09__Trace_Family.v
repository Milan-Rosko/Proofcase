...
└──────────────────────────────────────────────────────────────────────────────┘
┌──────────────────────────────────────────────────────────────────────────────┐
│                      Proofcase / P002_09__Trace_Family                       │
└──────────────────────────────────────────────────────────────────────────────┘
...
(*
│
│          This file starts the honest family-level FM reduction. Its
│          output is a finite cubic family whose satisfiability should
│          mirror one FM trace witness exactly; there is no longer any
│          family-to-single-equation compression step in scope.
│
*)
...
(*
│
│          `FMFixedTracePayloadFrom` is the general raw trace payload
│          from an arbitrary start code. `FMFixedTracePayload` is the
│          input-specialized version used by the public compiler
│          route.
│
*)
(*
│
│          `FMFixedTracePayload` is the exact fixed-witness semantic
│          object that the trace-family emitter must mirror. It
│          exposes the raw arithmetic payload of a valid witness: one
│          fuel bound whose iterated `StepCode` trace encodes to the
│          given witness and whose last code decodes to a halted
│          state.
│
*)
(*
│
│          `FMTraceSearchPayload` is the open existential version used
│          by the family compiler: it internalizes both the witness
│          code and the fuel bound into the valuation, while keeping
│          compilation syntax-driven.
│
*)
(*FMFixedTracePayload(prog,input,W,T)\;\equiv\;W=\encode\_code\_trace(code\_run\_trace(StepCode(prog),T,start\_code(input)))*)
(*\;\wedge\;halted\_state(decode\_state(code\_run\_last(StepCode(prog),T,start\_code(input)))).*)
(*
│
│          The fixed trace-channel indexing is now concrete. Variable
│          `0` is reserved for the designated final halt code, and
│          variable `S t` stores the code at time `t`.
│
*)
...
(*
│
│          The trace-channel predicates are the semantic shorthands
│          that make the block theorems readable. They describe what
│          the valuation is supposed to mean: a run-shaped sequence of
│          code channels, the fixed start code at time `0`, local
│          successor alignment with `StepCode`, and a designated final
│          halted code.
│
*)
(*   trace\_channels\_match\_run(prog,input,W,\rho)\Rightarrow \forall t\le   *)
(*            fuel,\ trace\_channel\_code(\rho,t)=coded\_nth(t,W).            *)
...
(*
│
│          `emit_trace_shape_family` is the first genuinely concrete
│          fixed-witness block. It does not inspect halting semantics;
│          it simply pins the trace-channel variables to the
│          primitive-recursive list decoder applied to the supplied
│          witness number `W`.
│
*)
(*
│
│          `emit_trace_family` is the fixed-witness emitter. It
│          receives a concrete witness code `W` and returns the local
│          FM constraints whose common satisfiability should mean
│          exactly `∃ fuel, FMFixedTracePayload(prog,input,W,fuel)`.
│
*)
...
(*
│
│          `emit_trace_family_wf` is the structural certification
│          theorem for the fixed-witness compiler block. It says the
│          emitted family is not merely satisfiable in the intended
│          cases; every atomic equation it emits is also well-scoped
│          with respect to the arities declared in `P002_00`.
│
*)
...
(*
│
│          `fixed_trace_payload_fuel_le_witness` is the numeric budget
│          fact needed for the future fixed-witness emitter. Once a
│          witness `W` really encodes a run of fuel `T`, the witness
│          number itself is large enough to bound all time indices
│          `0..T`. This keeps later witness decoding syntax-driven and
│          primitive recursive on `W`.
│
*)
(*
│
│          `fixed_trace_payload_decodes_trace` is the exact adequacy
│          bridge for fixed-witness compilation: if `W` is a genuine
│          trace witness, then the primitive-recursive decoder on `W`
│          recovers the concrete run trace that the later
│          shape/start/step/halt emitters should mirror.
│
*)
