...
└──────────────────────────────────────────────────────────────────────────────┘
  ...
  arithmetic substrate of the Vector Iterant machine layer.
...
(*
│
│          `FM_Z0` is the D001-facing alias of the canonical
...
(*
│
│          `FM_encode_pair` and `FM_decode_pair` re-export the
│          carryless pairing codec under machine-local names so
...
(*
│
│          `FM_Z0_sound` states that the imported support extractor is
│          numerically exact: summing the extracted support
...
(*
│
│          `FM_Z0_valid` complements `FM_Z0_sound`: the imported
│          support is not only exact, but also Zeckendorf-valid, which
│          is the admissibility condition required by the later
│          machine constructions.
│
*)

(*                         ∀ n, zeck_valid(FM_Z0(n)).                         *)
...
(*
│
│          `FM_pair_roundtrip` transfers the carryless roundtrip law
│          into the machine namespace: encoding a pair and then
│          decoding the resulting code returns the original
│          coordinates.
│
*)

(*           ∀ a b, FM_decode_pair(FM_encode_pair(a, b)) = (a, b).            *)
...
(*
│
│          `FM_pair_injective` is the corresponding uniqueness
│          principle at the machine layer: equal codes force equality
│          of both encoded coordinates.
│
*)

(*      FM_encode_pair(a, b) = FM_encode_pair(a', b') ⇒ a = a' ∧ b = b'.      *)
