...
└─────────────────────────────────────────────────────────────────────────┘
  ...
  This file establishes the foundational layer. It contains the Fibonacci
  arithmetic, the concrete Zeckendorf support engine, and the structural
  lemmas later used by the pairing and unpairing layers.
...
(*
│
│             `sum_fib` evaluates a finite support of
│             Fibonacci indices by summing the corresponding
│             Fibonacci values. This is the numeric reading
│             map used throughout the carryless encoding.
│
*)
...
(*
│
│             `unpair` is the abstract recovery map attached
│             to a parameter package `P`. It reconstructs the
│             left component from the even support and then
│             reconstructs the right component from the odd
│             support above the recovered boundary.
│
*)
...
(*
│
│             `zeck_valid` is the admissibility predicate for
│             Zeckendorf supports. It enforces strict
│             descent, exclusion of adjacent indices, and the
│             lower bound `2` on all used indices.
│
*)
...
(*
│
│             `r0(x)` is the first Fibonacci cutoff strictly
│             above `x`. The greedy constructor only needs to
│             search below this index.
│
*)
...
(*
│
│             `Z0` is the concrete greedy Zeckendorf support.
│             It computes the canonical non-adjacent
│             Fibonacci index set that later feeds the
│             pairing and decoding layers.
│
*)
...
(*              k ≤ j < find_r_aux(x,k,fuel) ⇒ fib(j) ≤ x.              *)
...
(*
│
│             r0(n) is the least Fibonacci index whose value
...
(*
│
│             `greedy_inv` packages the greedy correctness
│             invariant: xs encodes the extracted support,
│             rem' is the leftover remainder, xs stays within
│             the current bound, and the remainder remains
...
(*
│
│             `zeck_greedy_down_correct_core` is the main
│             recursive engine lemma. It shows that every
│             branch of the greedy descent produces a
│             support/remainder pair satisfying the full
│             invariant package `greedy_inv`.
│
*)
...
(*
│
│             `zeck_greedy_down_correct` is the public
│             wrapper around the core recursion lemma. It
│             exposes the invariant without committing
│             downstream proofs to the internal case split on
│             `k` and `prev`.
│
*)
...
(*
│
│             `Z0_sound` is the basic adequacy theorem for
│             the concrete support extractor: reading the
│             greedy support back through `sum_fib` returns
│             exactly the original number.
│
*)
...
(*
│
│             `Z0_valid` complements `Z0_sound`: the
│             extractor produces a canonical admissible
│             support, not merely a summing support.
│
*)
...
(*
│
│             `Zeckendorf_unique_core` is the canonicality
│               theorem for valid supports: equal Fibonacci
│             sums force equality of the supports themselves.
│
*)

(*             zeck_valid(xs) ∧ zeck_valid(ys) ∧ sum_fib(xs)            *)
(*                               ⇒ xs = ys                              *)
...
(*
│
│             `Z0_of_sum_fib` is the converse of `Z0_sound`:
│             every valid support is fixed by the extractor.
│
*)
...
(*
│
│             The next block transports Zeckendorf
│             admissibility through the even and odd
│             embedding maps that define the pairing support.
│
*)
...
(*
│
│             `odd_even_concat_valid` is the structural
│             compatibility lemma for pairing. The two bands
│             remain sufficiently separated that their
│             concatenation is again a valid Zeckendorf
│             support.
│
*)
...
(*
│
│             The “carryless” code therefore admits a
│             canonical odd/even support decomposition.
│
*)
...
(*
│
│             The final filtering corollaries recover the two
│             pairing bands directly from the canonical
│             support of a paired code.
│
*)
...
(*         ∀ x y, filter is_even (Z0(pair(x,y))) = even_band(x)         *)
...
(*  ∀ x y, filter (odd_ge_B1(B(x))) (Z0(pair(x,y))) = odd_band(x,y).    *)
