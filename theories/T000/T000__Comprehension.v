...
(*
  ...
  ======================================
    ...
    --------
      ...
      (i)
      CORE ARITHMETIC DEFINITION LAYER

        `R01` explains how to split a positive number into two meaningful parts:
        the odd piece that remains after all factors of `2` have been removed,
        and the count of how many times `2` was removed. This gives the basic
        arithmetic viewpoint used in the rest of the package.

      (ii)
      BOUNDS AND STRUCTURE

        odd_part_aux `R01` records the auxiliary boundedness, oddness, and
        divisibility lemmas that control the fuel-indexed odd-part recursion.

      (iii)
      PUBLIC-FACING PROPERTIES

        `R01` proves that the public odd-part map is odd, bounded, positive, and
        divisive on positive inputs.

      (iv)
      DECOMPOSITION AND DIVISIBILITY BRIDGE

        `R01` establishes the factorization `n = 2^(val2 n) * odd_part n` and
        derives `same_odd_part_divides`.

      (v)
      PIGEONHOLE CODOMAIN CONSTRUCTION

        `R01` constructs the explicit codomain `odd_range n` of odd numbers in
        `{1, ..., 2n}`, and proves its length, membership characterization,
        bounds, non-duplication, and the landing theorem `odd_part_in_range`.

      (vi)
      LIST-BASED PIGEONHOLE PRINCIPLE

        `R02` proves the abstract list pigeonhole theorem and combines it with
